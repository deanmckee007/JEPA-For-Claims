...
## 🧩 Conceptual Artifact Alignment

Codex agents **must reference `/core_artifacts/`** when present:
- Match code changes to implementation plans
- Update affected artifacts if functional scope changes
- Check downstream impact of vocabulary structure, sampling strategies, and masking logic

---

## ✅ Code Change Checklist

- [ ] Matches generator type (`use_diffusion` flag)
- [ ] Maintains `Config`-driven modularity
- [ ] Claims, codes, and time tokens are processed and masked correctly
- [ ] Losses are weighted per `diffusion_weight`, `level_2_weight`, and `log_vars`
- [ ] Vocab size and embeddings passed correctly to all components

---

## 🛠️ Suggested Codex Commands

- `Add a joint training step to combine VICReg and diffusion losses.`
- `Refactor model to condition diffusion on patient representation.`
- `Replace SAE with PCA projection in downstream evaluation.`
- `Update config to disable dropout for debugging.`
