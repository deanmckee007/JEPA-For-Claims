...
EPOCH_PATTERN = re.compile(r"epoch(?:epoch=)?(\d+)", re.IGNORECASE)


def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Evaluate a series of stage checkpoints with the cost probe."
    )
    parser.add_argument("--checkpoint-dir", required=True, help="Directory containing checkpoint files.")
    parser.add_argument(
        "--pattern",
        default="*.ckpt",
        help="Glob pattern used to select checkpoints inside checkpoint-dir.",
    )
    parser.add_argument(
        "--recipe",
    ...
    parser.add_argument(
        "--output-json",
        type=str,
        default="checkpoint_series_cost_probe.json",
...
def checkpoint_epoch_key(path: Path):
...
def main(argv=None):
    ...
    checkpoints = sorted(checkpoint_dir.glob(args.pattern), key=checkpoint_epoch_key)
    if not checkpoints:
        raise FileNotFoundError(
            f"No checkpoints matching pattern {args.pattern!r} under {checkpoint_dir.resolve()}"
        )
    ...
    for checkpoint_path in checkpoints:
        report = evaluate_checkpoint(
            checkpoint_path,
            config=config,
            train_probe_dataloader=train_probe_dataloader,
    ...
    best_by_rmse = min(results, key=lambda item: item["target_probe_rmse_dollars"])
    best_by_mae = min(results, key=lambda item: item["target_probe_mae_dollars"])
    best_by_wape = min(results, key=lambda item: item["target_probe_wape_percent"])
    best_by_q4_rmse = min(
        results,
        key=lambda item: item["target_cost_bucket"]["q4_high_cost"]["target_probe_rmse_dollars"],
    )
    ...
    summary = {
        ...
        "results": results,
        "best_by_target_probe_rmse_dollars": best_by_rmse,
        "best_by_target_probe_mae_dollars": best_by_mae,
        "best_by_target_probe_wape_percent": best_by_wape,
        "best_by_q4_high_cost_rmse_dollars": best_by_q4_rmse,
